// Add or remove conditions on a selected token
...
(async () => {
    ...
    if (!target) return ui.notifications.error(`Pas de token sélectionné`);
    ...
    let the_content = `<form><div><p>Conditions actuelles sur la cible :</p>${conditionsApplied}</div><hr><p>Choisissez une condition à ajouter ou à supprimer.</p>
...
<ul style="list-style:none;margin:0;padding:0;">${conditionOptions}</ul></div></div>
    ...
    let x = new Dialog({
        ...
        content: the_content,
        buttons: {
            apply: { label: "Ajouter", callback: () => (apply = true) },
            remove: { label: "Supprimer", callback: () => (remove = true) },
            all: { label: "Purge", callback: () => (all = true) },
        },
        close: async (html) => {
            if (apply) {
                const con_list = [];
